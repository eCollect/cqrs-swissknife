...
const asyncParamApiCallbacks = {
	...
	2: (fn, api) => (par1, par2, callback) => Promise.resolve(fn(par1, par2, api(par1))).then(result => callback(null, result), error => callback(error)),
	3: (fn, api) => (par1, par2, par3, callback) => Promise.resolve(fn(par1, par2, par3, api(par1))).then(result => callback(null, result), error => callback(error)),
};
